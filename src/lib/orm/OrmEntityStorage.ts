...
export const STORAGE_NAME_DB_TYPE = 'DbType';
...
export class OrmEntityStorage {
    ...
    public set(className: string, name: string, value: any): void {
        ...
        const storageTypes = this._argsMap.get(className);
...
export function ShardTable(shardCount: number = 1): Function {
    return (target: any) => {
        OrmEntityStorage.instance.set(target.name, STORAGE_NAME_SHARD_TABLE, Math.min(10, shardCount));
    };
}
...
// DbType
export function DbType(dbType: string = null): Function {
    return (target: any) => {
        OrmEntityStorage.instance.set(target.name, STORAGE_NAME_DB_TYPE, dbType);
    };
}