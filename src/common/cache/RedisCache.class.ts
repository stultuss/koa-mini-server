...
export class RedisCache extends AbstractCache {
    ...
    public async keys(pattern: string): Promise<string[]> {
        if (pattern == '*') {
            throw new ErrorFormat(700011, pattern);
        }
    ...
    public async flush(): Promise<boolean> {
        if (process.env.PROJECT_ENV == 'production') {
            throw new ErrorFormat(700010, process.env.PROJECT_ENV, 'redis.flush()');
        }
    ...
    public async hGet(key: string, field: number | string): Promise<any> {
        if (key == null || field == null) {
            CommonTools.logger(`[REDIS WARING] key:${key}, field ${field}`, CommonTools.LOGGER_TYPE_WARN);
            return null;
        }
